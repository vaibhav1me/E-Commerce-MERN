    ...
    <nav className="bg-primary p-[1rem]">
      <div className="large-menu hidden medium:block">
        <div className="flex justify-between items-center">
          ...
          <div id="" className="flex items-center">
            {user ? (
              <div className="">
                <span className="text-[1.2rem] px-[.2rem] text-yellow">
                  Hi! {user.name}
                </span>
              </div>
            ) : (
        ...
        {user ? (
          <div className="flex justify-between items-center mt-[.5rem] pt-[1rem] text-yellow">
            <div className="flex text-yellow">
              <NavLink
                to="/cart"
                id="cart"
                className="cursor-pointer text-center relative mx-2"
              >
                <span className="text-yellow font-bold bg-secondary rounded-full w-[1.5rem] inline-block absolute -top-[.8rem] -right-[.5rem]">
                  {user.cart.length}
                </span>
            ...
            <button
              id="logout"
              className="cursor-pointer hover:bg-secondary px-3 py-1 rounded-md"
              onClick={logoutUser}
            >
              Logout
            </button>
      ...
      <div className="mobile-menu relative block medium:hidden">
        <div className="flex justify-between items-center">
          <div
            id="logo"
            className="text-yellow text-[1.3rem] underline underline-offset-4"
          >
            <NavLink to="/">ShopAseZ</NavLink>
          </div>
          {user ? (
            ...
            <div>
              <NavLink to="/login" className="text-yellow">
                <button>Login</button>
              </NavLink>
            </div>
          )}
        ...
        <div
          className={`${
            toggle ? "block" : "hidden"
          } text-yellow bg-primary absolute top-[6rem] -right-[1rem] w-[10rem] p-3 rounded-lg border-yellow border-[2px] flex flex-col z-10 items-center justify-center`}
        >
          <NavLink
            to="/account"
            className="my-1"
          ...
          <NavLink to="/cart" className="my-1" onClick={() => setToggle(false)}>
            My Cart
          </NavLink>
          <button
            id="logout"
